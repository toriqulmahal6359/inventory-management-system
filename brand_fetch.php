<?php
    ...
    $query .= "SELECT * FROM brand 
        INNER JOIN category WHERE category.category_id = brand.category_id";
    
    if(isset($_POST["search"]["value"]))
    {
        $query .= 'WHERE brand.brand_name LIKE "%'.$_POST["search"]["value"].'%" ';
        $query .= 'OR category.category_name "%'.$_POST["search"]["value"].'%"';
    ...
    $sub_array[] = '<button typr="button" name="delete" id="'.$row["brand_id"].'" data-status=".$row["brand_status"]." class="btn btn-danger btn-xs delete">Delete</button>';
    $data[] = $sub_array;
